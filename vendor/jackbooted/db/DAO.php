...
abstract class DAO extends \Jackbooted\Util\JB {
    ...
    private function toWhere ( $where, &$params=null ) {
        if ( is_array ( $where ) && count ( $where ) > 0 ) {
            ...
            foreach ( $where as $key => $value ) {
                ...
                if ( $this->ignoreCols != null && array_key_exists( $key, $this->ignoreCols ) ) continue;
    ...
    public function insert ( $row, $insertMethod='INSERT' ) {
        ...
        if ( DB::exec ( $this->db, $sql, $values ) != 1 ) return false;
    ...
    public function objToRel ( $object ) {
        ...
        if ( is_array ( $object ) ) {
            foreach ( $object as $key => $val ) {
                if ( isset ( $this->orm[$key] ) ) $key = $this->orm[$key];
                $relational[$key] = $val;
            }
        }
        return $relational;
